  ...
  <div class="dashboard-container">
    <div class="container">
      ...
      >
        ...
        <el-table-column prop="category.name" label="套餐分类" />
...
import { setMealPageList, setMealDelete, setMealUpdateStatus } from "@/request/api";
...
const checkList = ref([]);
const tableData = ref([]);
const  setMealState = ref("");

init();

function init() {
  setMealPageList({ page: page.value, pageSize: pageSize.value, name: input.value })
    .then((res) => {
...
function addFoodtype(st: string) {
  if (st == "add") {
    ...
    router.push({ path: "/setmeal/add", query: { id: st } });
  }
}
...
function statusHandle(row: any) {
  ...
  if (typeof row === "string") {
   ...
   setMealState.value = params;

  console.log( setMealState.value);
  

  setMealUpdateStatus( setMealState.value)
    .then((res) => {
      if(res.code==200){
